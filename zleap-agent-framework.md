...
3. Result event 在 `userId + workspaceId` 范围内召回最新有效约 10 条，形成较早任务的结果时间线。
...
- **Result Event**：载入当前 user + workspace 下最新有效的约 10 条结果事件，作为较早任务的结果时间线。
...
- `readMemory` 是普通记忆的渐进式披露工具。自动召回和 `searchMemory` 只暴露紧凑投影，并明确标出 `summary_only`、`detailInjected=false` 与可读取详情的 id；当用户主动要求回忆、摘要不足以回答、或需要核对某条 impression/event 的完整内容时，模型用 `readMemory(memoryId)` 读取详情。如果用户在摘要回答后追问“详细说说”“展开讲讲”“具体一点”等，模型必须先读详情，而不是把摘要扩写成事实。这个触发应通过系统提示词和工具 schema 教会 agent 主动完成，不通过 runtime 强制 `tool_choice`；模型仍要自己从可见投影中选择最相关的 memoryId。
...
- `currentWorkspaceResultEvents`：当前 workspace 的结果事件时间线，保留过去约 10 条“完成了什么 / 失败了什么 / 产出了什么”的摘要。
...
系统提示词必须把主动读取讲成明确操作规程：用户先问“你认识我吗”时，agent 可以基于 summary 简短回答；用户继续说“详细说说”时，下一步应是 `readMemory` function call。反例也要写清楚：只根据 title/summary 扩写用户经历、项目、时间线或旧事件细节，属于记忆幻觉。
...
- Event memory 分两层注入：约 10 条 result event 保留旧结果时间线，少量 process event 通过 FTS / 未来向量按当前任务相关性召回，但只注入 id/title/summary/readMemory 等投影，不注入 detail/detailSnippet。
