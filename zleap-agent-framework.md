...
上下文堆栈是给人验收和排查用的界面，不应该把可解析 JSON 直接当原始文本倾倒出来。`tools`、`memory`、`history`、`tool_result` 等结构化内容应被解析成表格或字段视图；尤其是 `tools` 数组，要能清楚看到每次 LLM 调用实际暴露了哪些 function、参数 schema、绑定来源和风险信息。原始日志模式不是额外追加的一栏，也不是把 1/2/3/4/5/6/7 分区逐个 raw 化，而是隐藏结构化编号堆栈，只直接展示 `final_messages` 的原始 JSON/text，不做结构化表格渲染，也不需要再次展开。
