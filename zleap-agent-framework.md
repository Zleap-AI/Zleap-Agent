...
5. **history**：本地对话片段、当前 WorkspaceTask、已完成 WorkspaceResult、近期工具证据；子 workspace 的历史按 workspace 持续，不因切回 main 而删除，但只保留同一 workspace 的本地记录。跨 workspace 内容只能作为 `handoffContext` 出现，不能把所有对话记录混成一个全局 history。
