...
- Runtime:
  ...
  - Workspace definitions include a `memoryPolicyJson` contract controlling event/skill recall and write behavior. Event recall is layered by runtime strategy rather than a single undifferentiated cap; skill recall still uses the workspace skill cap.
  ...
  - Prompt injection must use projected memory views, not raw `MemoryRow` records. The LLM should receive compact ids/titles/summaries and safe metadata such as event kind/outcome/procedure; raw `detail`, full `metadataJson`, evidence arrays, and long source transcripts stay in SQLite, workspace sessions, tool calls, and audit/debug views.
  ...
  - Agent-side memory tools are registered into every workspace: `searchMemory`, `writeUserImpression`, `writeAgentSelfImpression`, and `writeSkillMemory`. There is no separate memory workspace, no event-memory write tool, and no model-callable update/delete memory tools.
  ...
  - Runtime policy prompts must include an explicit memory-write protocol: use `writeUserImpression` only for stable user preferences/background/identity/constraints, use `writeSkillMemory` only for user/agent-triggered reusable desensitized methods, do not write event memory directly because lifecycle hooks own event extraction, and require creator authorization for `writeAgentSelfImpression`.
  - `searchMemory` is the only model-callable memory management/read tool. Memory evolution is append-first: new facts or lessons create new records or new relation/version rows, and runtime recall reads the latest effective record. Update/delete are not exposed to the model.
  ...
  - Runtime memory tool schemas must not expose scope-moving fields. `searchMemory` exposes only query plus optional `memoryType`; write tools expose content and quality fields only. `memoryType`, `userId`, `agentId`, `workspaceId`, `relationId`, `version`, lifecycle evidence, and tenant scope are code/API-controlled fields.
...
- Runtime:
  ...
  - Runtime memory tools are limited to search plus impression/skill writes: `searchMemory`, `writeUserImpression`, `writeAgentSelfImpression`, and `writeSkillMemory`. `writeEventMemory`, memory update, and memory delete are not model-callable tools.
...
- Memory:
  ...
  - The model-callable runtime memory surface is intentionally small: `searchMemory`, `writeUserImpression`, `writeAgentSelfImpression`, and `writeSkillMemory`. Event writes, record updates, and deletes are not runtime tools.
