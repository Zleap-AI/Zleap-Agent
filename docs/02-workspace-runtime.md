...
当前实现以 `ZLEAP_MASTER_PLAN.md` 为准：首版内置 workspace 是 `main` 和统一的 `dev`，没有独立 `memory workspace`，Browser workspace 只作为未来扩展示例。memory 能力以通用 runtime tools 的形式挂载到每个 workspace 中。
...
Main Workspace
  -> enter Dev Workspace
     -> Dev Workspace searches files and/or runs commands
     -> Dev Workspace returns result
