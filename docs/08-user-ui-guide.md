...
- 本轮运行中默认保持折叠，只显示最近进展摘要和运行状态；用户点击后才能展开详情；`done` 后自动折叠。
- 切换历史会话回来时，运行过程必须可从 trace 重建，并保持可展开查看。
- 运行过程展开必须使用受控的 button/section 交互，不能依赖用户级消息流里的原生嵌套 `details` / `summary`；点击“查看详情”不得改变路由、清空 React root 或让聊天主界面白屏。
...
- 展开态显示步骤列表和每步的可读详情；详情必须直接在面板内呈现，不把原始 JSON 或技术 payload 作为主要展示。
