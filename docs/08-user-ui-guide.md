...
- 左侧显示当前 Agent 名称。
- 右侧显示当前生效模型、设置按钮、开发者模式入口。
- 如果系统没有真实的模型 catalog 数据源，Header 不得硬编码模型下拉列表；只能显示当前模型，点击后进入设置中的 Model 输入框。
- 不做 hero，不显示大标题，不出现营销文案。
- Header 的会话标题必须和主消息列表状态一致；如果当前主区是空对话状态，不得显示历史会话标题，只显示“新对话”。
...
- 运行过程必须按本轮 assistant message id 分组绑定，不能使用一个全局过程列表再锚到最新消息；后续对话不得把旧步骤搬到新消息上方。
- 运行过程面板使用与助手消息一致的头像列和内容列，右侧内容宽度必须与助手正文对齐。
- `enterWorkspace` 和 `exitWorkspace` 是内部边界事件，普通用户界面不得直接显示这些英文工具名；应自动识别为工作空间窗口的开始和结束。
- 外层运行过程展开后，按工作空间绘制矩形窗口，窗口标题显示工作空间名称和状态；同一个窗口只包含进入该工作空间后、退出该工作空间前发生的操作。
- `exitWorkspace` 后必须截断当前工作空间窗口；如果后续进入另一个工作空间，必须开始新的工作空间矩形。
- 本轮运行中默认保持折叠，只显示最近进展摘要和运行状态；用户点击后只能先展开步骤列表；单个步骤被点击后才展开该步骤详情；`done` 后自动折叠。
- 切换历史会话回来时，运行过程必须可从 trace 重建，并保持可展开查看。
- 运行过程和单步详情展开必须使用受控的 button/section 交互，不能依赖用户级消息流里的原生嵌套 `details` / `summary`；点击“查看详情”不得改变路由、清空 React root 或让聊天主界面白屏。
- 开发者模式不得在消息列表底部额外渲染全局运行过程列表；所有运行过程都必须绑定到对应 assistant message 上方，避免重复展示和旧过程串到新消息。
...
- 弹窗外层关闭区域必须透明，不得使用灰色、模糊或变暗遮罩遮住主聊天区。
...
- 展示当前工作空间可用的专属工具列表。
- 内置文件工具不得默认操作项目根目录，也不得使用项目内 `.codex` 或系统临时目录作为默认位置；每个会话必须使用专属工作目录，默认目录名形如 `~/Documents/Zleap/conversations/<conversationId>-<hash>/`，Windows 下对应用户文档目录里的 `Documents\Zleap\conversations\<conversationId>-<hash>\`。
...
- 外层展开态按工作空间窗口显示步骤列表，不直接显示工具调用细节；每个步骤可再点击展开可读详情；详情必须在该步骤下方呈现，不把原始 JSON 或技术 payload 作为主要展示。
...
6. `delta`：更新助手消息文本。
7. `workspace`：普通模式收集到当前 assistant message id 对应的轻量运行过程面板，不插入为普通聊天消息；开发者模式可展开查看更详细条目。
...
11. 刷新或切换历史会话：拉取 trace，按消息时间窗口把过程重建到对应助手消息上方，默认折叠但允许展开。
12. `error`：移除失败中的 assistant 占位或标记失败，显示重试入口。

### 页面刷新与标题状态

- 页面刷新后必须恢复当前会话 id，并立即加载该会话消息；不能只恢复 sidebar 选中态。
- 如果恢复的会话 id 已不存在，回退到最近会话并加载消息；如果没有历史会话，则创建本地空会话。
- 当主消息列表为空时，header subtitle 必须显示“新对话”，即使 localStorage 或 sidebar 中仍存在历史会话标题。
...
- Sidebar 顶部控制按钮必须用图标表达固定状态，而不是文字或方向箭头；固定状态使用 pin 图标，临时浮层/未固定状态使用浮动窗口图标，并通过 `aria-label` / `title` 说明“点击固定”或“点击取消固定”。
...
- Drawer 的右侧点击关闭区域从 drawer 右边缘开始，位置随 drawer 宽度变化。
