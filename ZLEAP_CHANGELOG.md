...
目的：
- 将用户级 UI 的运行过程按工作空间分组展示，避免把 `enterWorkspace` / `exitWorkspace` 这类内部编排事件直接暴露给普通用户。

变更：
- 更新 `src/web/main.tsx`：自动识别工作空间进入/退出边界，把边界之间的步骤渲染到对应工作空间窗口中；`exitWorkspace` 后截断当前窗口，后续工作空间重新开一个窗口。
- 更新 `src/web/styles.css`：新增工作空间窗口样式，使运行过程像分段窗口一样展示，并保持宽度与消息列对齐。
- 更新 `src/tests/run-tests.ts`：增加前端契约断言，确保运行过程存在工作空间窗口分组和边界识别逻辑。
- 更新 `docs/08-user-ui-guide.md`：补充 `enterWorkspace` / `exitWorkspace` 只能作为内部边界事件，普通用户界面必须显示工作空间窗口而不是英文工具名。

验证：
- 已执行 `npm run typecheck`、`npm run test`、`npm run build`，均通过。
- 已在浏览器检查 `http://localhost:4174/`：展开运行过程后显示“搜索工作空间”“开发工作空间”两个窗口，默认不显示 `enterWorkspace` / `exitWorkspace`，默认单步详情数量为 0；点击单步后详情数量为 1，页面无横向滚动。
...
目的：
- 调整运行过程展开层级：外层展开后只显示步骤列表，不直接展示工具调用详细字段。

变更：
- 更新 `src/web/main.tsx`：新增单步详情展开状态；外层运行过程面板打开时会清理该面板内的单步详情状态；每个步骤改为可点击按钮，只有点击该步骤后才显示可读详情。
- 更新 `src/web/styles.css`：为步骤按钮补齐 hover、打开态和按钮重置样式。
- 更新 `src/tests/run-tests.ts`：增加前端契约断言，确保运行过程详情是单步二次展开。
- 更新 `docs/08-user-ui-guide.md`：明确运行过程外层展开只显示步骤列表，步骤详情必须二次点击后展示。

验证：
- 已执行 `npm run typecheck`、`npm run test`、`npm run build`，均通过。
- 已在浏览器检查 `http://localhost:4174/`：外层展开后只显示步骤按钮，`.tool-activity-detail` 为 0；点击 `writeFile 返回：302ai_report.html` 单步后才出现 1 个详情块，控制台无 error。
...
目的：
- 调整用户级 UI 的运行过程面板：默认不展开工具调用详情，只显示轻量摘要。

变更：
- 更新 `src/web/main.tsx`：新一轮对话创建运行过程面板时默认折叠；运行中不再因为 active 状态自动展开；用户点击摘要行后仍可手动展开或收起。
- 更新 `docs/08-user-ui-guide.md`：明确运行中默认只展示最近进展摘要和运行状态，详情必须由用户主动展开。

验证：
- 已执行 `npm run typecheck`、`npm run test`、`npm run build`，均通过。
- 已在浏览器检查 `http://localhost:4174/`：历史会话加载后运行过程默认折叠，只显示摘要；点击过程摘要后可展开详情，控制台无 error。

Git：
- 本次提交：让运行过程详情默认折叠。

## 2026-06-01 22:06 +08:00

目的：
- 修复用户级 UI 点击运行过程“查看详情”后页面白屏的问题。

变更：
- 更新 `src/web/main.tsx`：用户级运行过程面板不再使用原生 `details` / `summary`，改为受控的 `button` + `section` 展开交互；步骤详情在面板展开后直接渲染为可读字段。
- 更新 `src/web/styles.css`：调整运行过程 summary 和步骤 summary 的样式选择器，保留宽度约束、截断和 hover 反馈。
- 更新 `docs/08-user-ui-guide.md`：明确运行过程详情展开必须使用稳定受控 UI，点击“查看详情”不得清空 React root 或让聊天主界面白屏。

验证：
- 已执行 `npm run typecheck`、`npm run test`、`npm run build`，均通过。
- 已在浏览器检查 `http://localhost:4174/`：点击第一条“运行过程 · 2 次搜索 / 7 个步骤”的“查看详情”后，聊天界面保持可见，面板展开并显示 7 条过程详情，控制台无 error。
...
目的：
- 修复用户级 UI 中运行过程使用全局列表并锚到最新消息的问题，避免第一轮步骤在后续对话中被挪到第三轮或更后面的记录里，导致列表越来越长。

变更：
- 更新 `src/web/main.tsx`：运行过程改为按 assistant message id 分组存储和渲染；流式 `workspace` 事件只追加到当前助手消息对应的过程面板；历史 trace 重新加载时按消息时间窗口把工具步骤绑定回对应助手消息。
- 开发者模式仍汇总展示全部过程，但普通消息流中每个过程面板只显示对应轮次的步骤。
- 更新 `docs/08-user-ui-guide.md`：明确运行过程必须按本轮助手消息绑定，不能用全局过程列表锚到最新消息。

验证：
- 已执行 `npm run typecheck`、`npm run build`、`npm run test`，均通过。
- 已在浏览器检查 `http://localhost:4174/`：打开包含两轮助手回复的历史会话后，消息列表顺序为 `user -> process -> assistant -> user -> process -> assistant`，运行过程不再集中显示到最新助手消息上方。
...
目的：
- 修复宽屏下左侧会话栏会沿用历史折叠状态、导致第一屏默认收起的问题。

变更：
- 更新 `src/web/main.tsx`：宽屏 `>=900px` 初始化时强制 sidebar 默认展开；窗口从窄屏切回宽屏时也恢复展开，并关闭移动抽屉状态。
- 更新 `docs/08-user-ui-guide.md`：明确 desktop 下 sidebar 默认展开，localStorage 中的折叠状态不应影响宽屏第一屏。

验证：
- 已执行 `npm run typecheck`、`npm run build`、`npm run test`，均通过。
- 已在浏览器检查 `http://localhost:4174/`：宽屏下先折叠 sidebar 后刷新，sidebar 会默认恢复展开，宽度为 `280px` 且无横向溢出。

Git：
- 本次提交：让宽屏默认展开 sidebar。

## 2026-06-01 18:04 +08:00

目的：
- 修复新用户 UI composer 左侧工具菜单只是空按钮的问题。

变更：
- 更新 `src/web/main.tsx`：工具菜单按钮现在可展开菜单，提供“工作空间与工具”“记忆”“Agent”“打开调试台”四个实际入口；点击入口会打开对应设置区或当前会话 `/dev` 调试页。
- 更新 `src/web/styles.css`：补充 composer 工具菜单弹层、菜单项和打开态样式，保证菜单文本不溢出且整行可点击。
- 更新 `docs/08-user-ui-guide.md`：明确 composer 工具菜单不能是无反馈空按钮，v1 至少提供工作空间与工具、记忆、Agent 和调试台入口。

验证：
- 已执行 `npm run typecheck`、`npm run build`、`npm run test`，均通过。
- 已在浏览器检查 `http://localhost:4174/`：工具菜单可展开为 4 个入口，点击“工作空间与工具”会打开设置弹窗对应区域，菜单关闭后无横向溢出。

Git：
- 本次提交：修复 composer 工具菜单。
