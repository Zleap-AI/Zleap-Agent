...
export function WorkScreen({ pane }: { pane: WorkPane | null }) {
  ...
  return (
    <div ref={scrollRef} onScroll={onScroll} className="soft-scroll flex-1 overflow-y-auto bg-console-screen px-3 py-3">
...
function WorkspaceTransitions({ pane }: { pane: WorkPane }) {
  ...
  return (
    <div className="animate-msg-in overflow-hidden rounded-md border border-border bg-surface/70">
      {transitions.map((transition, index) => (
        <div
          key={`${transition.fromSpace}:${transition.toSpace}:${transition.status}:${transition.createdAt}:${index}`}
          className="flex items-start gap-2 border-b border-border/60 px-3 py-2 text-[13px] last:border-b-0"
        >
          <ArrowRight className="mt-0.5 h-3.5 w-3.5 shrink-0 text-primary" />
          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 flex-wrap items-center gap-1.5 leading-5 text-ink">
              <span className="font-medium">{transition.fromSpace}</span>
              <span className="text-muted-foreground">→</span>
              <span className="font-medium">{transition.toSpace}</span>
              <span className="rounded-sm bg-muted px-1.5 py-0.5 text-[11px] leading-4 text-muted-foreground">{transition.status}</span>
            </div>
...
function WorkScreenStatus({ text, running }: { text: string; running: boolean }) {
  return (
    <div className="animate-msg-in flex items-center gap-2 rounded-md border border-border bg-surface/70 px-3 py-2 text-[13px] text-muted-foreground">
...
function WorkScreenMessage({ text }: { text: string }) {
  ...
  return (
    <div className="animate-msg-in overflow-hidden rounded-md border border-border bg-surface/60">
      <button
        type="button"
      ...
      >
        ...
        {!open ? (
          <div className={clsx('min-w-0 flex-1 text-[13px] leading-6 text-muted-foreground', 'line-clamp-2')}>
            {preview || text}
          </div>
        ) : (
          <div className="min-w-0 flex-1 truncate text-[13px] leading-6 text-muted-foreground">{preview || text}</div>
        )}
        {expandable ? (
          <ChevronDown
...
function EmptyScreen({ title, text, running = false }: { title: string; text: string; running?: boolean }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center bg-console-screen px-6 text-center">
      <span className="mb-3 flex h-9 w-9 items-center justify-center rounded border border-border bg-surface-2">
