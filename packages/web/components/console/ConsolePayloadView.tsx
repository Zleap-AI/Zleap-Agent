...
export function ConsolePayloadView({ value, label, codeLang, artifactSources = [] }: ConsolePayloadViewProps) {
  ...
  return (
    <div className="space-y-2">
      {label ? <div className="px-0.5 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">{label}</div> : null}
      {artifactRefs.length ? (
      ...
      {resolved.incomplete ? (
        <div className="rounded-md border border-amber-500/30 bg-amber-500/5 px-2.5 py-1.5 text-xs text-amber-700 dark:text-amber-300">
          数据不完整（可能是旧会话截断记录）。请开新对话重跑任务以查看完整内容。
        </div>
      ) : null}
...
function ArtifactRefCard({ item }: { item: WorkspaceArtifactRef }) {
  ...
  return (
    <div className="flex items-start gap-2 rounded-md border border-border bg-surface-2 px-3 py-2 text-[13px]">
      <div className="min-w-0 flex-1">
        <div className="font-medium text-ink">{title}</div>
        {item.kind ? <div className="mt-0.5 text-xs text-muted-foreground">{item.kind}</div> : null}
        {item.ref ? (
          <div className="mt-1 break-all font-mono text-[11px] text-muted-foreground" title={item.ref}>
            {path ?? item.ref}
          </div>
        ) : null}
      </div>
      {href ? (
        <a href={href} target="_blank" rel="noreferrer" className="shrink-0 text-muted-foreground hover:text-primary" title="打开产物">
...
function PayloadBody({
  ...
  return (
    <div className="overflow-x-auto rounded-sm border border-border bg-surface-2 px-3 py-2 font-mono text-[13px] leading-6 text-muted-foreground">
      {body.split('\n').map((line, index) => (
