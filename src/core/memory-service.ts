...
function findRawSourcePayloadKey(value: unknown): string | undefined {
  ...
  if (Array.isArray(value)) {
    for (const item of value) {
      const found = findRawSourcePayloadKey(item);
  ...
  for (const [key, item] of Object.entries(value as Record<string, unknown>)) {
    if (rawPayloadKeys.has(key)) return key;
    const found = findRawSourcePayloadKey(item);
    if (found) return found;
  }
  return undefined;
...
function sourceRef(table: string, ids: Array<string | undefined>): { table: string; ids: string[] } {
  return {
    table,
    ids: ids.filter((id): id is string => typeof id === "string" && id.length > 0)
  };
}

function sourceRefIds(metadata: Record<string, unknown>, table: string): string[] {
  ...
  const ref = refs.find((item) => (
    ...
    typeof item === "object" &&
    !Array.isArray(item) &&
    (item as Record<string, unknown>).table === table
...
export class MemoryService {
  ...
  private runtimeMemoryEvidence(activeWorkspaceSessionId?: string, activeTaskId?: string): Record<string, unknown> {
    return {
      ...
      ...(activeWorkspaceSessionId ? { workspaceSessionId: activeWorkspaceSessionId } : {})
    };
  }
  ...
  private maybeWriteConversationWindowEvent(run: AgentRunInput, workspaceId: string): MemoryRow[] {
    ...
    for (let windowIndex = 1; windowIndex <= completedWindows; windowIndex += 1) {
      ...
      const evidenceBase = {
        ...
        windowEndAt,
        sourceRefs: [
          sourceRef("messages", evidenceMessageIds),
          sourceRef("workspace_sessions", relatedSessions.map((session) => session.id)),
          sourceRef("tool_calls", windowToolCalls.map((toolCall) => toolCall.id))
        ],
        autoGenerated: true
      };
      ...
      if (!this.repos.getMemoryByRelation("event", resultRelationId, eventRelationScope)) {
        const resultMemory = this.createOrSkip({
          ...
          detail: [
            ...
            relatedSessions.length ? `关联工作空间结果：${relatedSessions.map((session) => `${session.workspaceId}:${session.status}:${compactText(session.summary || session.result.summary, 180)}`).join("；")}` : ""
          ].filter(Boolean).join("\n"),
          metadataJson: JSON.stringify({
            ...evidenceBase,
            eventKind: "result",
  ...
  private workspaceExitEvidence(run: AgentRunInput, session: WorkspaceSession): Record<string, unknown> {
    ...
    return {
      ...
      sourceRefs: [
        sourceRef("messages", messages.map((message) => message.id)),
        sourceRef("workspace_sessions", [session.id]),
        sourceRef("tool_calls", toolCalls.map((toolCall) => toolCall.id))
      ],
  ...
  private toolCallsForEventMemories(run: AgentRunInput, eventMemories: MemoryRow[]): ToolCallLog[] {
    ...
    for (const memory of eventMemories) {
      const metadata = this.parseMetadata(memory.metadataJson);
      for (const value of sourceRefIds(metadata, "tool_calls")) {
        if (value.trim()) ids.add(value);
      }
    }
  ...
  private memoryWriteDecision(memory: MemoryWriteInput, actorId: string, actorRole: UserRole): PolicyDecision {
    ...
    const rawPayloadKey = findRawSourcePayloadKey(this.parseMetadata(memory.metadataJson ?? "{}"));
    if (rawPayloadKey) {
      return { allowed: false, reason: `Memory metadata must store source table/id references instead of raw source payloads. Remove metadata.${rawPayloadKey}.` };
    }
  ...
  private auditMemoryHooks(run: AgentRunInput, activeWorkspaceId: string, writes: MemoryRow[]): void {
    for (const memory of writes) {
      ...
      if (metadata.source === "afterConversationWindow" || metadata.source === "afterWorkspaceExit") {
        ...
        const evidenceMessageIds = sourceRefIds(metadata, "messages");
        const workspaceSessionIds = sourceRefIds(metadata, "workspace_sessions");
        const toolCallIds = sourceRefIds(metadata, "tool_calls");
