...
export class MemoryService {
  ...
  private maybeWriteConversationWindowEvent(run: AgentRunInput, workspaceId: string): MemoryRow[] {
    ...
    for (let windowIndex = 1; windowIndex <= completedWindows; windowIndex += 1) {
      ...
      const evidenceBase = {
        ...
        sourceRefs: [
          { table: "messages", ids: evidenceMessageIds },
          { table: "workspace_sessions", ids: relatedSessions.map((session) => session.id) },
          { table: "tool_calls", ids: windowToolCalls.map((toolCall) => toolCall.id) }
        ],
  ...
  private workspaceExitEvidence(run: AgentRunInput, session: WorkspaceSession): Record<string, unknown> {
    ...
    return {
      ...
      sourceRefs: [
        { table: "messages", ids: messages.map((message) => message.id) },
        { table: "workspace_sessions", ids: [session.id] },
        { table: "tool_calls", ids: toolCalls.map((toolCall) => toolCall.id) }
      ],
